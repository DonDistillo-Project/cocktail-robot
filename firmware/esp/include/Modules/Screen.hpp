...
RenderMessageTextData alloc_message_text_data(unsigned char len,
                                              const char *text) {
  RenderMessageTextData data{.len = len,
...
const Box shrink_box(const Box box, unsigned int pixels) {
  return Box{.left = box.left + pixels,
             .top = box.top + pixels,
             .right = box.right - pixels,
             .bottom = box.bottom - pixels};
}
...
void draw_string_in_box(UWORD Xstart, UWORD Ystart, const char *pString,
                        const sFONT *Font, UWORD Color_Background,
                        UWORD Color_Foreground, const Box *box) {
  UWORD Xpoint = Xstart;
  UWORD Ypoint = Ystart;
...
void draw_string_rel_box(const char *pString, const sFONT *Font,
                         UWORD Color_Background, UWORD Color_Foreground,
                         const Box *box, unsigned int offset_x = 0,
                         unsigned int offset_y = 0) {
  draw_string_in_box(offset_x + box->left, offset_y + box->top, pString, Font,
                     Color_Background, Color_Foreground, box);
}

void draw_box(const Box *box, UWORD Color, DOT_PIXEL Line_width,
              DRAW_FILL Filled) {
  Paint_DrawRectangle(box->left, box->top, box->right, box->bottom, Color,
                      Line_width, Filled);
}
const unsigned int PopUpMargin = 20;
const unsigned int PopUpBorderSize = 3;
const Box PopUpBorderBox = {.left = PopUpMargin,
                            .top = PopUpMargin,
                            .right = LCD_WIDTH - PopUpMargin,
                            .bottom = LCD_HEIGHT - PopUpMargin};
const Box PopUpBox = shrink_box(PopUpBorderBox, PopUpBorderSize);

const sFONT *RecipeHeaderFont = &Font8;
const Box RecipeHeaderBox = {.left = 5,
                             .top = 2,
                             .right = LCD_WIDTH - 5,
                             .bottom =
                                 RecipeHeaderFont->Height * (unsigned int)2};

const sFONT *RecipeFont = &Font20;
const unsigned int RecipeLines = 2;
const Box RecipeBox = {.left = RecipeHeaderBox.left,
                       .top = RecipeHeaderBox.bottom,
                       .right = RecipeHeaderBox.right,
                       RecipeHeaderBox.bottom +
                           RecipeLines * RecipeFont->Height};

const sFONT *InstructionFont = &Font16;
const unsigned int InstructionLines = 8;
const unsigned int InstructionBoxBorderSize = 2;
const Box InstructionBorderBox = {
    .left = RecipeBox.left,
    .top = RecipeBox.bottom + InstructionBoxBorderSize,
    .right = RecipeBox.right,
    .bottom = RecipeBox.bottom + InstructionLines * InstructionFont->Height +
              2 * InstructionBoxBorderSize};
const Box InstructionBox =
    shrink_box(InstructionBorderBox, InstructionBoxBorderSize);

const sFONT *ProgressStringFont = &Font16;
const unsigned int ProgressStringPadding = 6;
const Box ProgressStringBox = {
    .left = InstructionBorderBox.left,
    .top = InstructionBorderBox.bottom + ProgressStringPadding,
    .right = InstructionBorderBox.right,
    .bottom = InstructionBorderBox.bottom + ProgressStringPadding +
              ProgressStringFont->Height};

const unsigned int ProgressBarBorderSize = 2;

const Box ProgressBarBorderBox = {
    .left = ProgressStringBox.left + ProgressBarBorderSize,
    .top = ProgressStringBox.bottom + ProgressBarBorderSize,
    .right = ProgressStringBox.right - ProgressBarBorderSize,
    .bottom = LCD_HEIGHT - ProgressBarBorderSize};
const Box ProgressBarBox =
    shrink_box(ProgressBarBorderBox, ProgressBarBorderSize);

const double ProgressBarMaxPercentage = 1.2;
const unsigned int ProgressBarWidth =
    ProgressBarBox.right - ProgressBarBox.left;
const unsigned int ProgressBarTargetWidth = 4;
const unsigned int ProgressBarTargetCenter =
    ProgressBarBox.left + ProgressBarWidth / ProgressBarMaxPercentage;

const Position get_text_size(const char *str, const sFONT *font,
                             unsigned int x_limit = LCD_WIDTH) {
  ...
  unsigned int max_width =
      min(max_chars_without_nl * font->Width, x_limit_rounded_down);
  unsigned int height =
      (line_count + len * font->Width / x_limit_rounded_down) * font->Height;

  return Position{.x = max_width, .y = height};
}
...
void draw_string_centered(const char *str, const Box *box, const sFONT *font,
  ...
  draw_string_in_box(box_center.x - text_size.x / 2,
                     box_center.y - text_size.y / 2, str, font, bg, fg, box);
}
...
void _render_recipe_layout() {
  ...
  draw_string_centered(recipe_header, &RecipeHeaderBox, RecipeHeaderFont, WHITE,
                       BLACK);
  vTaskDelay(0);

  // Draw Scale Outline
  draw_box(&ProgressBarBorderBox, BLACK, (DOT_PIXEL)ProgressBarBorderSize,
           DRAW_FILL_EMPTY);
  vTaskDelay(0);

  // Draw Target Line
  Paint_DrawRectangle(
      ProgressBarTargetCenter - ProgressBarTargetWidth / 2, ProgressBarBox.top,
      ProgressBarTargetCenter + ProgressBarTargetWidth / 2,
      ProgressBarBox.bottom, BLUE, DOT_PIXEL::DOT_PIXEL_1X1, DRAW_FILL_FULL);
  vTaskDelay(0);

  // Draw Instruction Seperator
  draw_box(&InstructionBorderBox, GRAYBLUE, DOT_PIXEL::DOT_PIXEL_2X2,
           DRAW_FILL::DRAW_FILL_EMPTY);
  vTaskDelay(0);
...
void _render_recipe(RecipeData recipe) {
  ...
  draw_string_rel_box(recipe.text, RecipeFont, WHITE, BLACK, &RecipeBox);
}
void _render_instruction(InstructionData instruction) {
  printf("Rendering Instruction: %s\n", instruction.text);

  Box clear_box = {.left = InstructionBox.left,
                   .top = InstructionBox.top,
                   .right = InstructionBox.right,
                   .bottom = InstructionBox.top + last_instruction_height};
  draw_box(&clear_box, WHITE, DOT_PIXEL::DOT_PIXEL_1X1,
           DRAW_FILL::DRAW_FILL_FULL);

  draw_string_rel_box(instruction.text, InstructionFont, WHITE, BLACK,
                      &InstructionBox);
  last_instruction_height =
      get_text_size(instruction.text, InstructionFont, &InstructionBox).y;
}
void _render_success(SuccessData success) {
  printf("Rendering Success: %s\n", success.text);
  draw_box(&PopUpBorderBox, GBLUE, (DOT_PIXEL)PopUpBorderSize,
           DRAW_FILL::DRAW_FILL_EMPTY);
  draw_box(&PopUpBox, GREEN, DOT_PIXEL::DOT_PIXEL_1X1,
           DRAW_FILL::DRAW_FILL_FULL);
  draw_string_rel_box(success.text, &Font24, GBLUE, WHITE, &PopUpBox);
...
void _render_error(ErrorData error) {
  ...
  draw_box(&PopUpBorderBox, GRED, (DOT_PIXEL)PopUpBorderSize,
           DRAW_FILL::DRAW_FILL_EMPTY);
  draw_box(&PopUpBox, RED, DOT_PIXEL::DOT_PIXEL_1X1, DRAW_FILL::DRAW_FILL_FULL);
  draw_string_rel_box(error.text, &Font24, BRED, WHITE, &PopUpBox);
...
void _render_scale(ScaleData scale) {
  ...
  size_t scale_buf_len =
      sprintf(scale_buf, "%4.1fg / %4.1fg", scale.value, scale.target);

  double fill_percent = scale.value / scale.target;
  double bar_width = ProgressBarBox.right - ProgressBarBox.left;

  double target_width =
      bar_width / ProgressBarMaxPercentage - ProgressBarTargetWidth / 2;
  ...
  draw_box(&ProgressStringBox, WHITE, DOT_PIXEL::DOT_PIXEL_1X1,
           DRAW_FILL::DRAW_FILL_FULL);

  draw_string_centered(scale_buf, &ProgressStringBox, &Font16, WHITE, BLACK);

  int value_x =
      min(target_start + (int)(fill_percent * target_width), target_end);
  int overflow_x = max(
      min(overflow_start +
              (int)(((fill_percent - 1.0) / (ProgressBarMaxPercentage - 1.0)) *
                    overflow_width),
          overflow_end),
      overflow_start);
  ...
  if (value_x > prev_value_x) // Fill up green bar to current level
  {
    ...
    Paint_DrawRectangle(prev_value_x, ProgressBarBox.top, value_x,
                        ProgressBarBox.bottom, GREEN, DOT_PIXEL::DOT_PIXEL_1X1,
                        DRAW_FILL_FULL);
  }

  if (overflow_x > prev_overflow_x) {
    // Extend overflow bar
    Paint_DrawRectangle(prev_overflow_x, ProgressBarBox.top, overflow_x,
                        ProgressBarBox.bottom, RED, DOT_PIXEL::DOT_PIXEL_1X1,
                        DRAW_FILL_FULL);
  }

  if (overflow_x < prev_overflow_x) {
    // Shrink overflow bar
    Paint_DrawRectangle(overflow_x, ProgressBarBox.top, prev_overflow_x,
                        ProgressBarBox.bottom, WHITE, DOT_PIXEL::DOT_PIXEL_1X1,
                        DRAW_FILL_FULL);
  }

  if (value_x < prev_value_x) {
    // Clear green bar for smaller current value
    Paint_DrawRectangle(value_x, ProgressBarBox.top, prev_value_x,
                        ProgressBarBox.bottom, WHITE, DOT_PIXEL::DOT_PIXEL_1X1,
                        DRAW_FILL_FULL);
  }
...
void render_recipe(unsigned char recipe_name_len, const char *recipe_name) {
  ...
  RenderMessage msg{.type = RenderMessageType::RENDER_MSG_RECIPE,
                    .data = {.recipe = alloc_message_text_data(recipe_name_len,
                                                               recipe_name)}};

  send_render_message(&msg);
}

void render_instruction(unsigned char instruction_len,
                        const char *instruction) {
  RenderMessage msg{.type = RenderMessageType::RENDER_MSG_INSTRUCTION,
                    .data = {.instruction = alloc_message_text_data(
                                 instruction_len, instruction)}};

  send_render_message(&msg);
}
void render_success(unsigned char success_msg_len, const char *success_msg) {
  RenderMessage msg{.type = RenderMessageType::RENDER_MSG_SUCCESS,
                    .data = {.success = alloc_message_text_data(success_msg_len,
                                                                success_msg)}};
  send_render_message(&msg);
}
void render_error(unsigned char error_msg_len, const char *error_msg) {
  RenderMessage msg{
      .type = RenderMessageType::RENDER_MSG_ERROR,
      .data = {.error = alloc_message_text_data(error_msg_len, error_msg)}};
  send_render_message(&msg);
}
void render_scale(double value, double target) {
  RenderMessage msg{
      .type = RenderMessageType::RENDER_VAL_SCALE,
      .data = {.scale = ScaleData{.value = value, .target = target}}};
  send_render_message(&msg);
}
...
void SetupScreen() {
  ...
  xTaskCreate(render_loop, "render_loop", SCREEN_RENDER_STACKSIZE, NULL,
              SCREEN_RENDER_PRIO, &render_task);
